 ...
 */
export function LazyImage({ src, alt, className = "", aspectRatio = "aspect-video" }: LazyImageProps) {
  // Flag indicating image loading success in the DOM
  const [loaded, setLoaded] = useState(false);

  return (
    <div className={`relative overflow-hidden bg-warmgray ${aspectRatio} ${className}`}>
      {/* 1. PULSING SHIMMER BACKDROP: Renders while assets list is loading */}
      {!loaded && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-full w-full animate-pulse bg-gradient-to-r from-warmgray via-stone-200 to-warmgray" />
        </div>
      )}
