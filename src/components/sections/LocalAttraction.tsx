...
export function LocalAttraction({
    currentLang,
    translations,
}: LocalAttractionProps) {
    // Category state filter: defaults to 'all' spots
    const [activeCategory, setActiveCategory] = useState<CategoryFilter>('all');

    const categoriesSet: { label: string; value: CategoryFilter }[] = [
        {
            label: currentLang === 'en' ? 'All Spots' : 'Semua Lokasi',
            value: 'all',
        },
        {
            label: currentLang === 'en' ? 'Yogyakarta Culture' : 'Budaya Jawa',
            value: 'culture',
        },
        { label: currentLang === 'en' ? 'Beaches' : 'Pantai', value: 'beach' },
        {
            label: currentLang === 'en' ? 'Bespoke Dining' : 'Kuliner Bantul',
            value: 'restaurant',
        },
    ];
    ...
    return (
        <section
            id="attractions"
            className="bg-cream py-16 px-6 md:px-12 text-stone-900 border-t border-stone-200/40"
        >
                    ...
                    {filteredAttractions.map((attSnippet) => (
                        ...
                        >
                            ...
                            <div className="sm:w-2/5 relative h-44 sm:h-auto overflow-hidden shrink-0">
                                ...
                                <span className="absolute top-3 left-3 bg-cream/90 border border-gold/25 text-gold text-[8px] font-mono tracking-widest uppercase font-bold py-1 px-2 rounded backdrop-blur-xs">
                                    {attSnippet.category}
                                </span>
                            </div>
