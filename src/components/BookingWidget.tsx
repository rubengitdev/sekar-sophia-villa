...
export const BookingWidget: FC<BookingWidgetProps> = ({
    ...
    const [priceBreakdown, setPriceBreakdown] = useState<
        { date: string; rate: number; isPremium: boolean; seasonName: string }[]
    >([]);
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < calculatedNights; i++) {
            ...
            breakdown.push({
                date: currentNightDate.toLocaleDateString(
    ...
    const handleBookingSubmit = (e: React.FormEvent) => {
        ...
        const guestName =
            fullName.trim() ||
            (currentLang === 'en' ? 'Valued Guest' : 'Tamu Terhormat');
...
Kirim otomatis pada: ${formattedInquiryDate}`;
    ...
    return (
        <div
            id="booking-widget"
            className="bg-white border border-stone-200/80 rounded-xl p-6 md:p-8 shadow-sm text-stone-900"
        >
            {/* Widget Header Area */}
            <div className="flex items-center space-x-3 mb-6">
                ...
                <div>
                    <h4 className="font-serif text-lg font-bold text-stone-900">
                        {currentLang === 'en'
                            ? 'Direct Stay Rates Calculator'
                            : 'Kalkulator Tarif Langsung'}
                    </h4>
                    <p className="font-mono text-[10px] tracking-wider uppercase text-stone-500 mt-0.5">
                        {currentLang === 'en'
                            ? 'Exclusive pricing - No broker fee'
                            : 'Harga khusus - Bebas komisi agen'}
                    </p>
                </div>
            </div>
                ...
                <div className="flex flex-col space-y-1">
                    <label className="text-[10px] font-mono tracking-wider uppercase text-stone-500 font-semibold flex items-center justify-between">
                        ...
                        <span className="text-[10px] text-stone-400 font-sans tracking-tight font-normal">
                            {translations.capacityLabel}: Max 6 Guests
                        </span>
                    </label>
                        ...
                        {priceBreakdown.map((item, idx) => (
                            ...
                            >
                                <span className="text-stone-500">
                                    {item.date}{' '}
                                    {item.isPremium && (
                                        <span className="text-[9px] bg-gold/10 text-gold border border-gold/30 px-1 rounded ml-1 font-mono tracking-wide font-semibold">
                                            {item.seasonName}
                                        </span>
                                    )}
                                </span>
