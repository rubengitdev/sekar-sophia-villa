 ...
 */
export function Navbar({ currentLang, onLanguageChange, translations, activeTab, setActiveTab }: NavbarProps) {
      ...
      <nav
        ...
        className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 transform px-6 md:px-12 ${
          ...
          isScrolled
            ...
            : "bg-[#fdfcfb]/70 py-4 text-stone-900 border-b border-stone-200/25 backdrop-blur-sm"
        }`}
      >
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <div className="hidden sm:flex items-center space-x-4">
            ...
            <div className="flex items-center bg-stone-100 rounded-full p-0.5 border border-stone-200">
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase transition-all cursor-pointer ${
                  currentLang === "en" ? "bg-stone-900 text-white shadow-xs" : "text-stone-500 hover:text-stone-850"
                }`}
              >
                EN
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded-full text-[10px] font-bold tracking-wider uppercase transition-all cursor-pointer ${
                  currentLang === "id" ? "bg-stone-900 text-white shadow-xs" : "text-stone-500 hover:text-stone-850"
                }`}
              >
                ID
              </button>
            ...
            >
              ...
              <span>{currentLang === "en" ? "Book Direct" : "Pesan Langsung"}</span>
            </button>
          </div>
          ...
          >
            ...
            <div className="flex flex-col items-center space-y-6 w-full max-w-xs">
              {navItems.map((item, index) => (
                <motion.button
                  ...
                  className={`font-sans text-sm uppercase tracking-widest cursor-pointer focus:outline-none w-full py-2.5 border-b border-stone-100 text-center ${
                    activeTab === item.id ? "text-gold font-bold" : "text-stone-600 hover:text-stone-900"
                  }`}
                >
                  {item.label}
                </motion.button>
              ...
              >
                ...
                >
                  {currentLang === "en" ? "Book Direct" : "Pesan Langsung"}
                </button>
              </motion.div>
            </div>
          </motion.div>
