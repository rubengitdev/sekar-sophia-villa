...
import { Maximize, Bed, Bath, Users, CheckCircle, ChevronLeft, ChevronRight } from "lucide-react";
 ...
 */
export function Accommodation({ currentLang, translations }: AccommodationProps) {
  ...
  const handlePrevImg = (e: React.MouseEvent) => {
    e.stopPropagation();
    setActiveImgIndex((prev) => (prev - 1 + activeRoom.images.length) % activeRoom.images.length);
  };
  ...
  return (
    <section id="accommodation" className="bg-[#fdfcfb] text-stone-900 py-16 px-6 md:px-12">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-wrap md:flex-nowrap justify-center gap-1.5 mb-10 bg-stone-100/90 border border-stone-200/80 rounded-xl p-1.5 max-w-xl mx-auto">
          {ROOMS.map((room) => (
            ...
            >
              {room.categoryName ? room.categoryName[currentLang] : (room.name[currentLang].split(" ").slice(-2).join(" "))}
            </button>
          ))}
        </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between">
            <div className="space-y-4">
              ...
              <div className="relative aspect-video md:aspect-[4/3] rounded-xl overflow-hidden border border-stone-200/80 shadow-xs group">
                <img
                  src={getDirectImageUrl(activeRoom.images[activeImgIndex] || activeRoom.images[0])}
                    ...
                    {activeRoom.images.map((_, idx) => (
                      <button
                        ...
                        className={`w-1.5 h-1.5 rounded-full transition-all ${
                          activeImgIndex === idx ? "bg-gold scale-125" : "bg-white/60 hover:bg-white"
                        }`}
              ...
              {activeRoom.images.length > 0 && (
                <div>
                  <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-widest mb-2 font-semibold">
                    {currentLang === "en" ? "Suite Perspectives" : "Pilihan Sudut Pandang"} ({activeRoom.images.length})
                  </span>
          ...
          <div className="lg:col-span-6 flex flex-col justify-center space-y-6 lg:pl-6">
            <div>
              <span className="text-[10px] font-mono text-gold tracking-[0.2em] uppercase font-semibold">
                {activeRoom.badgeName ? activeRoom.badgeName[currentLang] : (activeRoom.id === "suite-master" ? "Premium Residence" : "Garden Retreat")}
              </span>
            ...
            <div className="grid grid-cols-2 gap-3 border-t border-stone-200/70 pt-6">
              <div className="flex items-center space-x-3 bg-stone-50/50 p-2.5 rounded-lg border border-stone-200/40">
                ...
                <div>
                  <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-wider">{translations.specSize}</span>
                  <span className="text-xs sm:text-sm font-semibold text-stone-850">{activeRoom.specs.size}</span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3 bg-stone-50/50 p-2.5 rounded-lg border border-stone-200/40">
                ...
                <div>
                  <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-wider">{translations.specBed}</span>
                  <span className="text-xs sm:text-sm font-semibold text-stone-850 line-clamp-1">{activeRoom.specs.beds[currentLang]}</span>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-3 bg-stone-50/50 p-2.5 rounded-lg border border-stone-200/40">
                ...
                <div>
                  <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-wider">{translations.capacityLabel}</span>
                  <span className="text-xs sm:text-sm font-semibold text-stone-850">{activeRoom.specs.capacity[currentLang]}</span>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-stone-200/70 pt-6">
              <span className="block text-[10px] font-mono text-stone-500 uppercase tracking-widest mb-3 font-semibold">
                {currentLang === "en" ? "Suite Premium Inclusions" : "Kelengkapan Premium Kamar"}
              </span>
              <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                {activeRoom.features[currentLang].map((feat, idx) => (
                  <div key={idx} className="flex items-center space-x-2 text-xs sm:text-sm font-sans text-stone-600">
